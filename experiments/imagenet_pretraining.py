...
from utils import anti_adversary_wrapper
...
def copy_pretrained_model(model, 
    ...
    resnet = torch.load(path_to_copy_from)
...
def get_imagenet_pretrained_model(k, alpha, dataset='cifar10'):
    if dataset == 'cifar10':
        WEIGHTS_PATH = './weights/cifar10wrn_baseline_epoch_4.pt'
        model = WideResNet(num_classes=10)
    else:
        WEIGHTS_PATH = './weights/cifar100wrn_baseline_epoch_4.pt'
        model = WideResNet(num_classes=100)
    model = copy_pretrained_model(model, WEIGHTS_PATH)
    model = anti_adversary_wrapper(model, mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5], k=k, alpha=alpha)
    model.to("cuda")
    return model
